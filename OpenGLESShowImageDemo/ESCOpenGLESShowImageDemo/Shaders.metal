...
// buffer 缓存数据修饰符，苹果内置不可变，ZYImageVertexInputIndexVertexs是索引
// [[buffer(ZYImageVertexInputIndexVertexs)]]
// constant 变量类型修饰符，表示存储在device区域

vertex RasterizerData vertexImageShader(uint vertexID [[vertex_id]], constant ZYVertex * vertexArray [[buffer(ZYImageVertexInputIndexVertexs)]]) {
    
    RasterizerData outData;
    
    // 获取ZYVertex里面的顶点坐标和纹理坐标
    outData.clipSpacePosition = vertexArray[vertexID].position;
    outData.clipSpacePosColor = vertexArray[vertexID].positionColor;
...
// ZYImageTextureIndexBaseTexture表示纹理索引

fragment float4 fragmentImageShader(RasterizerData input [[stage_in]], texture2d<half> baseTexture [[ texture (ZYImageTextureIndexBaseTexture) ]]) {
    ...
    half4 arcColor = half4(input.clipSpacePosColor);
    half4 newColor = (arcColor * color) / 1.0;
    // 返回颜色值
